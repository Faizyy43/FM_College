...
export default function EsInstallmentSection({ student }) {
  ...
  const installmentAmount =
    payments.length > 0 ? payments[payments.length - 1].amount : totalFees;

  const totalInstallments =
    installmentAmount > 0 ? Math.ceil(totalFees / installmentAmount) : 0;

  const paidInstallments =
    installmentAmount > 0 ? Math.floor(paidFees / installmentAmount) : 0;
