...
//   ahmedabad: {
//     label: "Ahmedabad",
//     data: AhmedabadColleges,
//   },
//   gandhinagar: {
//     label: "Gandhinagar",
//     data: GandhinagarColleges,
//   },
//   vadodara: {
//     label: "Vadodara",
//     data: VadodaraColleges,
//   },
//   rajkot: {
//     label: "Rajkot",
//     data: RajkotColleges,
...
//   // If URL city is valid, use it; otherwise default to first key
//   const defaultCityKey = Object.keys(cityConfig)[0];
//   const selectedCity =
//     citySlug && cityConfig[citySlug] ? citySlug : defaultCityKey;

//   const colleges = cityConfig[selectedCity].data || [];
...
//             Colleges in {cityConfig[selectedCity].label}
//           </h1>
//         </header>
...
//             No colleges found for this city.
//           </div>
//         ) : (
...
//                           📍 <span className="align-middle">{college.address}</span>
//                         </p>

//                         {Array.isArray(college.main_courses) &&
//                           college.main_courses.length > 0 && (
//                             <div className="mb-3 flex flex-wrap gap-1.5">
//                               {college.main_courses.slice(0, 5).map((course, i) => (
//                                 <span
//                                   key={i}
...
//   // Only use citySlug if it exists in cityConfig
//   const selectedCity = citySlug && cityConfig[citySlug] ? citySlug : null;

//   const colleges = selectedCity ? cityConfig[selectedCity].data : [];

//   return (
//     <div className="min-h-screen bg-slate-100 text-slate-900">
//       <div className="max-w-6xl mx-auto px-4 py-8">
//         {!selectedCity ? (
//           <div className="mt-20 text-center text-gray-600 text-lg">
//             Please select a valid city to view colleges.
//           </div>
//         ) : (
//           <>
//             {/* Header */}
    ...
    <div className="min-h-screen bg-white text-slate-900">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-amber-100 bg-white/80 px-3 py-2 text-xs md:text-sm shadow-sm mb-3">
          ...
          <div>
            <p className="text-slate-700">
              Viewing colleges in <span className="font-semibold capitalize">{cityName}</span>.
            </p>
            <p className="text-[11px] text-slate-500">
              Click a card to open the detailed page for that college.
            </p>
        ...
        <p className="text-xs text-slate-500 mb-3 mt-3">
          Showing{" "}
          <span className="font-semibold text-slate-700">{colleges.length}</span>{" "}
          colleges in <span className="font-semibold text-slate-700 capitalize">{cityName}</span>
        </p>
        ...
        {colleges.length === 0 ? (
          ...
          <div className="grid gap-5 grid-cols-1">
            {colleges.map((college, idx) => {
              const slug = slugify(college.name);
              return (
                <Link key={`${college.name}-${idx}`} to={`/college/${citySlug}/${slug}`}>
                  <article className="group flex h-full cursor-pointer flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-all hover:border-blue-500 hover:shadow-lg">
                    <div className="flex gap-10">
                      {/* Logo */}
                      {college.logo && (
                      ...
                      <div className="flex flex-col flex-1">
                        ...
                        {/* Courses */}
                        {Array.isArray(college.main_courses) && college.main_courses.length > 0 && (
                          <div className="mb-3 flex flex-wrap gap-1.5">
                            {college.main_courses.slice(0, 5).map((course, i) => (
                              <span
                                key={i}
